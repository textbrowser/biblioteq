  ...
  QMainWindow(), qtbook_item(rowArg)
{
  ...
  httpProgress->reset(); // Qt 5.5.x adjustment.
  ...
  if((thread = new(std::nothrow) generic_thread(this)) != 0)
	 ...
	 !thread->getZ3950Results().isEmpty())
	{
	  if(QMessageBox::question
		...
		m.initialize(marc::BOOK, marc::Z3950, marc::MARC21);
	      else
		m.initialize(marc::BOOK, marc::Z3950, marc::UNIMARC);

	      m.setData(thread->getZ3950Results()[0]);
...
void qtbook_book::slotDownloadFinished(bool error)
{
  Q_UNUSED(error);
  downloadFinished();
}
...
void qtbook_book::downloadFinished(void)
{
  if(httpProgress->isVisible())
    httpProgress->hide();
...
void qtbook_book::slotSRUDownloadFinished(bool error)
{
  Q_UNUSED(error);
  sruDownloadFinished();
}
...
void qtbook_book::sruDownloadFinished(void)
{
  m_sruWorking->hide();
  update();
  ...
  */
  ...
  if(!m_sruResults.isEmpty())
			       ...
			       QMessageBox::No) == QMessageBox::Yes)
	{
	  ...
	  if(recordSyntax == "MARC21")
	    m.initialize(marc::BOOK, marc::SRU, marc::MARC21);
	  else
	    m.initialize(marc::BOOK, marc::SRU, marc::UNIMARC);
...
bool qtbook_book::useHttp(void) const
{
#if QT_VERSION < 0x050000
#ifdef Q_OS_MAC
